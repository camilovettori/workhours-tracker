...
# =========================
# App / Paths (Render-friendly)
# =========================
BASE_DIR = Path(__file__).resolve().parent
STATIC_DIR = BASE_DIR / "static"

# Render: configure a persistent disk and set DATA_DIR=/var/data
...
def send_reset_email(to_email: str, reset_link: str) -> None:
    ...
    user = os.environ.get("SMTP_USER")          # for SendGrid usually: "apikey"
    pw = os.environ.get("SMTP_PASS")            # your SendGrid API key
    from_email = os.environ.get("FROM_EMAIL")   # e.g. "Work Hours Tracker <you@gmail.com>"
...
def last_monday(year: int, month: int) -> date:
    if month == 12:
        ...
        d0 = date(year, month + 1, 1) - timedelta(days=1)
    while d0.weekday() != 0:
        d0 -= timedelta(days=1)
    return d0
...
def signup(payload: SignupIn):
    ...
    with db() as conn:
        ...
        # migrate old legacy data (user_id=0) only if first user ever
        if before == 0:
...
def create_week(payload: WeekCreate, request: Request):
    uid = require_user(request)
    try:
        parse_ymd(payload.start_date)  # expects YYYY-MM-DD
    except Exception:
